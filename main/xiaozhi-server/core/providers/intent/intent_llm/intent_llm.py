    ...
    def get_intent_system_prompt(self) -> str:
        ...
        for key, value in self.intent_options.items():
            if key == "play_music":
                ...
                intent_list.append("4.声纹注册, 用户想要注册声纹, 比如说'我叫小明，我要注册声纹（文）'、'我叫小明，我要注册声纹（文）'等")
            else:
        ...
        prompt = (
            ...
            "{intent: '播放音乐 [获取的音乐名字]'}\n"
            "5. 属于4的意图，如果用户提到了名字，则返回格式如下：\n"
            "{intent: '声纹注册 [用户名]'}\n"
            "如果用户没有提到名字，请返回："
            "{intent: '需要更多信息：请告诉我您的名字以完成声纹注册'}"
            "6. 对于用户的名字，他可能对自己的名字进行解释怎么写"
            ...
            "```"
            "用户: 我想注册声纹（文），我叫小明\n"
            "思考(不返回): 用户表达了想注册声纹（文）的意图，因为STT的问题文和纹都是可以的，并提供了名字'小明'，属于声纹注册的意图\n"
            "返回结果: {intent: '声纹注册 [小明]'}\n"
            "```"
