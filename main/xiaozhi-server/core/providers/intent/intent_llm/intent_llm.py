...
class IntentProvider(IntentProviderBase):
    ...
    def get_intent_system_prompt(self) -> str:
        ...
        "register_voiceprint": "4.声纹注册, 用户想要注册声纹, 比如说'我叫小明，我要注册声纹'、'帮我注册声纹，我是小明'等"
        """
        for key, value in self.intent_options.items():
            ...
            elif key == "register_voiceprint":
                intent_list.append("4.声纹注册, 用户想要注册声纹, 只用于注册声纹的意图")
            else:
        ...
        prompt = (
            ...
            "{intent: '播放音乐 [获取的音乐名字]'}\n"
            "5. 属于4的意图，无论用户是否说出自己的名字，都需要询问他的名字\n"
            "{intent: '请告诉我您的名字以完成声纹注册'}\n"
            "6. 如果用户说出自己的名字，你需要从他的回答中，准确提取出他的名字，对于其他无关内容直接忽视\n"
            "{intent: '声纹注册 [用户名]'}\n"
            # "7. 对于用户的名字，他可能对自己的名字进行解释怎么写"
            ...
            "```"
            "用户: 我想注册声纹，我叫小明\n"
            "思考(不返回): 用户表达了想注册声纹的意图，并提供了名字'小明'，属于声纹注册的意图\n"
            "返回结果: {intent: '声纹注册 [小明]'}\n"
            "```"
            # "用户: 我想注册声纹（文），我叫王玥，月是左边一个王，右边一个月\n"
            # "思考(不返回): 用户表达了想注册声纹（文）的意图，后面半句话是对名字的解释，你需要理解这个字，并返回名字'王玥'，属于声纹注册的意图\n"
            # "返回结果: {intent: '声纹注册 [王玥]'}\n"
            # "```"
